...
@implementation CNSAnimatedGIFFrame
...
@implementation CNSAnimatedGIFQueueObject
...
@implementation CNSAnimatedGIFLoader

static CNSAnimatedGIFLoader * instance;
...
+ (CNSAnimatedGIFLoader *) sharedInstance {
  if (instance == nil)
  {
    instance = [[CNSAnimatedGIFLoader alloc] init];
  }
...
+ (UIImageView *) getAnimationForGifAtUrl:(NSURL *)animationUrl {   
  CNSAnimatedGIFQueueObject *agqo = [[CNSAnimatedGIFQueueObject alloc] init];
  [agqo setUiv:[[[UIImageView alloc] init] autorelease]]; // 2x retain, alloc and the property.
  [agqo setUrl: animationUrl]; // this object is only retained by the queueobject, which will be released when loading finishes
  [[CNSAnimatedGIFLoader sharedInstance] addToQueue: agqo];
  [agqo release];
  
  if ([[CNSAnimatedGIFLoader sharedInstance] busyDecoding] != YES)
  {
    [[CNSAnimatedGIFLoader sharedInstance] setBusyDecoding: YES];
    
    // Asynchronous loading for URL's, else the GUI won't appear until image is loaded.
    [[CNSAnimatedGIFLoader sharedInstance] performSelector:@selector(asynchronousLoading) withObject:nil afterDelay:0.0];
  }
...
- (void) asynchronousLoading {
  ...
  {
    NSData *data = [NSData dataWithContentsOfURL: [(CNSAnimatedGIFQueueObject *) [imageQueue objectAtIndex: 0] url]];
...
- (void) addToQueue: (CNSAnimatedGIFQueueObject *) agqo {
...
- (NSData*) getFrameAsDataAtIndex:(int)index {
	...
	{
		return ((CNSAnimatedGIFFrame *)[GIF_frames objectAtIndex:index]).data;
	}
	else
	{
		return nil;
...
- (UIImageView*) getAnimation {
    ...
    else
    {
      imageView = [[UIImageView alloc] initWithImage:[self getFrameAsImageAtIndex:0]];
		...
		int i = 0;
		CNSAnimatedGIFFrame *lastFrame = nil;
		for (UIImage *image in array) {
			CNSAnimatedGIFFrame *frame = [GIF_frames objectAtIndex:i];
			if (lastFrame) {
    ...
    [array release];
		...
		double total = 0;
		for (CNSAnimatedGIFFrame *frame in GIF_frames) {
			total += frame.delay;
		}
...
- (void)GIFReadExtensions {
	...
	unsigned char cur[1] = { 0x00 };
  unsigned char prev[1] = { 0x00 };
  [self GIFGetBytes:1];
	...
	while (cur[0] != 0x00)
  {
		...
		if (cur[0] == 0x04 && prev[0] == 0xF9)
		{
			...
			CNSAnimatedGIFFrame *frame = [[CNSAnimatedGIFFrame alloc] init];
...
- (void) GIFReadDescriptor {	
  ...
  // Deep copy
	...
	CNSAnimatedGIFFrame *frame = [GIF_frames lastObject];
	frame.area = rect;
